...
async def scrape_jiji(page: Page, listing_type: str) -> list[dict]:
    ...
    for page_num in range(1, MAX_PAGES + 1):
        ...
        next_btn = await page.query_selector(
            "a.b-pagination__right, [class*='pagination-next'], [aria-label='Next page']"
        )
        if not next_btn:
            log.info(f"[{src}] No more pages after page {page_num}")
            break
...
def deduplicate(listings: list[dict]) -> list[dict]:
    seen, unique = set(), []
    for rec in listings:
        key = rec.get("url") or (rec.get("title", "") + rec.get("price", ""))
...
def save_csv(listings: list[dict], filepath: str):
    Path(filepath).parent.mkdir(parents=True, exist_ok=True)
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=FIELDS, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(listings)
    log.info(f"Saved {len(listings)} listings → {filepath}")
...
async def main():
    ...
    log.info("=" * 60)
    log.info("  Nairobi Property Scraper v2 (Playwright)")
    log.info(f"  Max pages per source: {MAX_PAGES}")
    ...
    async with async_playwright() as pw:
        ...
        await context.route(
            "**/{analytics,gtm,facebook,hotjar,doubleclick}**",
            lambda route: route.abort()
        )
        ...
        # ── BuyRentKenya ────────────────────────────────────
        if SOURCES.get("buyrentkenya"):
            all_listings.extend(await scrape_buyrentkenya(page, "sale"))
            await delay()
            all_listings.extend(await scrape_buyrentkenya(page, "rent"))
            await delay()

        # ── Property24 ──────────────────────────────────────
        if SOURCES.get("property24"):
            all_listings.extend(await scrape_property24(page, "sale"))
            await delay()
            all_listings.extend(await scrape_property24(page, "rent"))
            await delay()

        # ── PigiaMe ─────────────────────────────────────────
        if SOURCES.get("pigiame"):
            all_listings.extend(await scrape_pigiame(page, "sale"))
            await delay()
            all_listings.extend(await scrape_pigiame(page, "rent"))
            await delay()

        # ── Jiji ────────────────────────────────────────────
        if SOURCES.get("jiji"):
            all_listings.extend(await scrape_jiji(page, "sale"))
            await delay()
            all_listings.extend(await scrape_jiji(page, "rent"))
    ...
    # ── Save ─────────────────────────────────────────────────
    save_csv(all_listings, OUTPUT_FILE)

    elapsed = (datetime.now() - start).total_seconds()
    log.info("=" * 60)
    log.info(f"  DONE — {len(all_listings)} unique listings in {elapsed:.1f}s")
    ...
    if all_listings:
        ...
        empty_title = df["title"].eq("").sum()
        empty_price = df["price"].eq("").sum()
        print(f"\nEmpty titles: {empty_title}  |  Empty prices: {empty_price}")
        print(f"Total: {len(df)} listings  →  {OUTPUT_FILE}\n")
